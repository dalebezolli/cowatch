...
func TestReflectRoomDetailsHandler(t *testing.T) {
	t.Run("host sending reflect room message with no vieweres", func(t *testing.T) {
		...
		receivedResponse := ReflectDetailsHandler(mockClient, mockManager, string(requestRoomReflection))
		assertExpectedMessageCount(t, 0, receivedResponse)
	})
	...
	t.Run("viewer sending reflect room message", func(t *testing.T) {
		...
		receivedResponse := ReflectDetailsHandler(mockViewer, mockManager, string(requestRoomReflection))
